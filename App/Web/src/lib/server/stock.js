...
export function aggregateStockEntries(entries = []) {
	...
	const totalQuantity = safeEntries.reduce((sum, entry) => sum + normalizeNumber(entry?.quantity, 0), 0);
...
export async function setProductStock({ readClient, writeClient, productId, quantity, unit, lowStockThreshold, now }) {
	...
	if (primary?._id) {
		...
		return { stockEntryId: primary._id, quantity, unit: nextUnit, lowStockThreshold: nextThreshold };
	}

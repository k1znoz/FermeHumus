...
<svelte:head>
  ...
  <meta
    name="description"
    content="Produits de saison de la Ferme de l'Humus : frais, transformés et plants. Commande en direct et retrait local."
  />
</svelte:head>
...
<div class="pt-24 pb-32 px-margin-mobile md:px-lg max-w-[1140px] mx-auto">
  ...
  <section class="mb-lg">
    ...
    <p class="font-body-lg text-body-lg text-on-surface-variant max-w-2xl">
      Produits de saison cultivés à la ferme et transformation artisanale sur place. Commandez en
      ligne, puis récupérez votre sélection sur marché ou sur rendez-vous.
    </p>
  </section>
  ...
  {:else}
    <section
      class="mb-lg rounded-2xl border border-[#2D4236]/12 bg-[#f8f7f3] p-lg md:p-xl shadow-ambient"
    >
      <div class="grid grid-cols-1 md:grid-cols-[auto_1fr] gap-4 items-start">
        <div
          class="w-12 h-12 rounded-full bg-[#e9e8e5] text-[#2D4236] flex items-center justify-center"
        >
          <span class="material-symbols-outlined">inventory_2</span>
        </div>
        <div>
          <p class="font-label-caps text-secondary uppercase tracking-widest mb-2">
            Un petit creux entre deux récoltes
          </p>
          ...
          <div class="flex flex-wrap gap-3">
            <a
              ...
              class="inline-flex items-center gap-2 rounded-xl bg-[#172c21] text-white px-5 py-2.5 text-sm font-semibold hover:bg-[#2d4236] transition-colors"
            >
            ...
            <a
              href="/contact"
              class="inline-flex items-center gap-2 rounded-xl border border-[#2D4236]/15 bg-white px-5 py-2.5 text-sm font-semibold text-primary hover:bg-surface-container-low transition-colors"
            >
  ...
  <section
    class="mt-6 mb-xl rounded-2xl overflow-hidden bg-surface-container-low border border-outline-variant/10 shadow-ambient"
  >
    <div class="grid grid-cols-1 md:grid-cols-[1.2fr_0.8fr] gap-gutter items-stretch">
      <div class="p-lg md:p-xl">
        <div class="flex items-center gap-3 mb-md">
          ...
          />
          <div>
            <span class="font-label-caps text-secondary uppercase tracking-widest block"
              >Vente directe</span
            >
            <h2 class="font-h2 text-h2 text-primary">
              Produits de saison, marchés et retrait local
            </h2>
          </div>
        </div>
        <p class="font-body-md text-on-surface-variant mb-lg max-w-xl">
          Les repères sont les mêmes partout: logo, affiche marché et photos réelles. Vous commandez
          ici, puis vous choisissez le point de retrait qui vous convient.
        </p>
        <div class="flex flex-wrap gap-3">
          <a
            href="/ou-nous-trouver"
  ...
  <section class="mb-xl">
    <div
      class="max-w-[1140px] mx-auto rounded-2xl bg-surface-container-low border border-outline-variant/10 shadow-ambient p-lg md:p-xl grid grid-cols-1 md:grid-cols-[1.25fr_0.75fr] gap-lg items-center"
    >
      <div>
        <span class="font-label-caps text-secondary uppercase tracking-widest block mb-sm"
          >Traiteur evenementiel</span
        >
        ...
        <p class="font-body-md text-on-surface-variant mb-md">
          Fabrication paysanne avec des produits locaux bio, cuits sur place au four a bois le jour
          J.
        </p>
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <a
            href="/images/ferme/tartes-flambees-evenements.pdf"
            target="_blank"
            rel="noreferrer"
            class="inline-flex items-center gap-2 rounded-xl border border-[#2D4236]/15 bg-white px-6 py-3 text-sm font-semibold text-primary hover:bg-surface-container-low transition-colors"
          >
      ...
      <div class="bg-white rounded-xl p-md shadow-ambient">
        <img
          ...
          alt="Prestation de la ferme pour des evenements locaux"
...
{#if cartCount > 0}
  <div
    class="fixed bottom-0 left-0 w-full bg-[#F5F5DC] border-t border-[#2D4236]/10 px-5 py-4 z-40"
  >
    <div
      class="max-w-[1140px] mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
    >
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="font-body-md text-on-surface-variant text-sm">
            {cartCount} article{cartCount > 1 ? 's' : ''} · {formatMoney(cartTotal)}
          </p>
        </div>
      </div>
      <button
...
<button
  ...
  aria-label="Réserver"
  form={cartCount > 0 ? 'product-order-form' : undefined}
  type={cartCount > 0 ? 'submit' : 'button'}
  disabled={submitting}
>
...
{#if cartCount > 0}
  <section class="px-margin-mobile md:px-lg mb-36" id="validation-panier">
    <div
      class="max-w-[1140px] mx-auto rounded-2xl border border-[#2D4236]/10 bg-white shadow-ambient overflow-hidden"
    >
      <div class="grid lg:grid-cols-[1.1fr_0.9fr]">
        <div class="p-lg md:p-xl bg-[#f5f5dc]">
          <p class="font-label-caps text-secondary uppercase tracking-widest mb-sm">
            Votre sélection
          </p>
          ...
          <div class="space-y-3">
            {#each cart as item (item.product._id)}
              <div
                class="flex items-center justify-between gap-4 rounded-xl bg-white px-4 py-3 border border-[#2D4236]/10"
              >
                <div class="min-w-0">
                  <p class="font-medium text-primary truncate">{item.product.name}</p>
                  <p class="text-sm text-on-surface-variant">
                    {item.quantity} × {formatMoney(item.product.price)} = {formatMoney(
                ...
                <div class="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    class="w-8 h-8 rounded-full border border-[#2D4236]/15 text-[#2D4236] hover:bg-white transition-colors"
          ...
          <div
            class="mt-6 flex items-center justify-between rounded-xl bg-white px-4 py-3 border border-[#2D4236]/10"
          >
          ...
          <p class="mt-4 text-sm text-on-surface-variant">
            La validation réserve les quantités en stock et transmet la commande à l'atelier.
          </p>
        </div>
        ...
        <div class="p-lg md:p-xl">
          ...
          >
            ...
            {#if form?.success}
              <div
                class="rounded-xl border border-[#2D4236]/15 bg-[#d0e8d7] px-4 py-3 text-sm text-[#172c21]"
              >
                <p class="font-semibold">
                  Commande validée {form.orderNumber ? `- ${form.orderNumber}` : ''}
                </p>
                <p class="mt-1">
                  Les quantités ont été réservées et la notification a été envoyée à l'atelier.
                </p>
              </div>
            {/if}
            ...
            <div class="grid md:grid-cols-2 gap-4">
              <div>
                <label for="customerName" class="block text-sm font-medium text-[#1a1c1a] mb-1"
                  >Nom et prénom *</label
                >
                <input
                  id="customerName"
              ...
              <div>
                <label for="customerEmail" class="block text-sm font-medium text-[#1a1c1a] mb-1"
            ...
            <div>
              <label for="customerPhone" class="block text-sm font-medium text-[#1a1c1a] mb-1"
            ...
            <div>
              <label for="pickupLocation" class="block text-sm font-medium text-[#1a1c1a] mb-1"
                >Où souhaitez-vous retirer votre commande ? *</label
              >
              <select
                id="pickupLocation"
                name="pickupLocation"
            ...
            <div>
              <label for="referralSource" class="block text-sm font-medium text-[#1a1c1a] mb-1"
                >Par curiosité, comment avez-vous entendu parler de nous ?</label
              >
              <select
                id="referralSource"
                name="referralSource"
            ...
            <div
              class="flex items-start gap-3 rounded-xl border border-[#2D4236]/10 bg-white px-4 py-3"
            >
              <input
                id="newsletterOptIn"
                name="newsletterOptIn"
                type="checkbox"
                class="mt-1 h-4 w-4 rounded border-[#2D4236]/30 text-[#172c21] focus:ring-[#172c21]"
              />
              <label for="newsletterOptIn" class="text-sm text-on-surface-variant"
                >Je souhaite être inscrit.e à la newsletter de la ferme.</label
              >
            </div>

            <div>
              <label for="notes" class="block text-sm font-medium text-[#1a1c1a] mb-1"
                >Questions, commentaires ou consignes</label
              >
              <textarea
                id="notes"
                name="notes"
                rows="4"
                placeholder="Informations utiles pour préparer la commande..."

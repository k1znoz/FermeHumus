...
<div class="max-w-md mx-auto px-5 pt-10 pb-6">
  ...
  <div
    class="mb-5 flex items-center justify-between gap-3 rounded-xl border border-[#e3e2e0] bg-white px-4 py-3"
  >
    <p class="text-sm text-[#424844]">Produit manquant dans la liste ?</p>
    <a
      href="/admin/products/new"
      class="shrink-0 rounded-lg bg-[#964824] px-3 py-2 text-xs font-semibold text-white hover:bg-[#76300d] transition-colors"
    >
      Nouveau produit
    </a>
  </div>
  ...
  {#if data.products.length === 0}
    ...
    >
      Aucun produit disponible pour la récolte. Créez d'abord un produit.
    </div>
  {/if}
  ...
  >
    <!-- Produit -->
    <div>
      <label for="productId" class="block text-sm font-medium text-[#1a1c1a] mb-1"
    ...
    <div class="grid grid-cols-2 gap-3">
      ...
      <div>
        <label for="unit" class="block text-sm font-medium text-[#1a1c1a] mb-1"
          >Unité <span class="text-red-500">*</span></label
        >
        <select
          id="unit"
          name="unit"
    ...
    <!-- Date -->
    <div>
      <label for="harvestDate" class="block text-sm font-medium text-[#1a1c1a] mb-1"
    ...
    <div>
      <label for="notes" class="block text-sm font-medium text-[#1a1c1a] mb-1"
        >Notes de qualité / N° de lot</label
      >
      <textarea
        id="notes"
        name="notes"
    ...
    <!-- Enregistré par -->
    <div>
      <label for="recordedBy" class="block text-sm font-medium text-[#1a1c1a] mb-1"
        >Enregistré par</label
      >
      <input
        id="recordedBy"
        name="recordedBy"
    ...
    <button
      type="submit"
      disabled={submitting || data.products.length === 0}
  ...
  <p class="mt-10 text-center text-xs text-[#737873]">
    Humus & Earth — Régénération & Transparence
  </p>
</div>

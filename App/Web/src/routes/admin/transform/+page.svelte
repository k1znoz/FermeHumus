...
<div class="max-w-md mx-auto px-5 pt-6 pb-6">
  ...
  <div
    class="rounded-2xl overflow-hidden border border-[#e3e2e0] mb-5 bg-white shadow-[0_4px_20px_rgba(45,66,54,0.05)]"
  >
    <div
  ...
  <form method="POST" class="space-y-4">
    <div class="rounded-2xl bg-white border border-[#e3e2e0] p-4 space-y-3">
        ...
        >
          {#each rawStocks as stock}
            <option value={stock.product?._id}
              >{stock.product?.name} ({stock.quantity} {stock.unit})</option
            >
          {/each}
        </select>
      </div>
      ...
      <div class="grid grid-cols-[1fr_auto] gap-3 items-end">
        <div>
          <label for="inputQuantity" class="block text-sm text-[#424844] mb-1"
            >Quantite consommee</label
          >
          <div class="grid grid-cols-[1fr_auto] rounded-xl overflow-hidden border border-[#e3e2e0]">
            ...
            />
            <span class="bg-[#f5f5dc] px-3 py-2.5 text-sm text-[#737873]"
              >{selectedRawStock?.unit ?? 'kg'}</span
            >
          </div>
        </div>
        <div class="rounded-xl bg-[#f4f3f1] px-3 py-2.5 border border-[#e3e2e0] min-w-[120px]">
          ...
          <p class="text-sm font-semibold text-[#172c21]">
            {selectedRawStock?.quantity ?? 0}
            {selectedRawStock?.unit ?? ''}
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="flex justify-center py-1">
      <div
        class="h-11 w-11 rounded-full bg-[#964824] text-white grid place-items-center shadow-[0_8px_18px_rgba(45,66,54,0.2)]"
      >
        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <div class="rounded-2xl bg-white border border-[#e3e2e0] p-4 space-y-3">
      ...
      <div>
        <label for="outputProductId" class="block text-sm text-[#424844] mb-1"
          >Produit transforme</label
        >
        <select
          id="outputProductId"
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          <label for="outputQuantity" class="block text-sm text-[#424844] mb-1"
            >Quantite produite</label
          >
          <input
            id="outputQuantity"
            name="outputQuantity"
    ...
    <div
      class="rounded-2xl bg-white border border-[#e3e2e0] p-4 grid grid-cols-[1fr_auto] items-end gap-3"
    >
      <div>
    ...
    <div class="rounded-2xl bg-white border border-[#e3e2e0] p-4 space-y-3">
      <div>
        <label for="batchNumber" class="block text-sm text-[#424844] mb-1"
          >Numero de lot (Batch)</label
        >
        <input
          id="batchNumber"
          name="batchNumber"
    ...
    {#if data.success}
      <p class="text-sm text-[#172c21] bg-[#d0e8d7] rounded-xl px-4 py-3">
        Transformation enregistree et stocks mis a jour.
      </p>
    {/if}

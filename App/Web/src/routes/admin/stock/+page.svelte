<script>
  ...
  const categories = $derived([
    'all',
    ...new Set(stocks.map((s) => s.product?.category).filter(Boolean)),
  ]);
  ...
  function isLow(stock) {
    ...
    return (
      ...
      Number(quantities[productId] ?? 0) <= stock.lowStockThreshold
    );
  }
...
<div class="max-w-md mx-auto px-5 pt-10 pb-4">
        ...
        <button
          ...
          onclick={() => {
            quantities = Object.fromEntries(
              stocks.map((stock) => [stock.product?._id, stock.quantity])
            );
          }}
  ...
  <div class="relative mb-4">
    <svg
      class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#737873]"
      fill="none"
      stroke="currentColor"
      stroke-width="2"
      viewBox="0 0 24 24"
  ...
  <div class="flex gap-2 overflow-x-auto pb-1 mb-5 scrollbar-none">
    {#each categories as cat}
      <button
        ...
        class="shrink-0 rounded-full px-4 py-1.5 text-xs font-semibold uppercase tracking-wide transition-colors {activeCategory ===
        cat
    ...
    <div class="space-y-3">
      {#each filtered as stock (stock.product?._id ?? stock._id)}
        ...
        >
          ...
          {:else}
            <div
              class="w-16 h-16 rounded-xl bg-[#e9e8e5] shrink-0 flex items-center justify-center"
            >
              <svg
                class="w-6 h-6 text-[#737873]"
                fill="none"
                stroke="currentColor"
                stroke-width="1.5"
                viewBox="0 0 24 24"
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs {low ? 'text-red-500 font-semibold' : 'text-[#737873]'}">
              ...
              {productId ? quantities[productId] : stock.quantity}
              {stock.unit}
            </p>
          </div>

...
<div class="max-w-md mx-auto px-5 pt-10 pb-6">
  <!-- En-tête -->
  <div class="mb-1">
    <p class="text-xs font-semibold text-[#964824] uppercase tracking-widest">
      Journal des mouvements
    </p>
    <h1 class="font-serif text-2xl font-semibold text-[#172c21]">Historique</h1>
    <p class="text-sm text-[#737873] mt-0.5">
      Récoltes et transformations enregistrées par l'équipe.
    </p>
  </div>
  ...
  <div class="flex gap-2 mt-5 mb-6 overflow-x-auto scrollbar-none">
    {#each [['all', 'Tous'], ['harvest', 'Récoltes'], ['transformation', 'Transformations'], ['order', 'Commandes']] as [val, label]}
      <button
        onclick={() => (filter = val)}
        class="shrink-0 rounded-full px-4 py-1.5 text-xs font-semibold uppercase tracking-wide transition-colors {filter ===
        val
              ...
              {#each entries as entry}
                <div class="flex items-start gap-4 pl-1">
                  ...
                  {#if entry.type === 'harvest'}
                    <div
                      class="w-9 h-9 rounded-full bg-[#d0e8d7] border-2 border-white flex items-center justify-center shrink-0 z-10"
                    >
                      <svg
                  ...
                  {:else if entry.type === 'transformation'}
                    <div
                      class="w-9 h-9 rounded-full bg-[#ffdbcd] border-2 border-white flex items-center justify-center shrink-0 z-10"
                    >
                      <svg
                        class="w-4 h-4 text-[#964824]"
                        fill="none"
                        stroke="currentColor"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                  ...
                  {:else}
                    <div
                      class="w-9 h-9 rounded-full bg-[#e7eefb] border-2 border-white flex items-center justify-center shrink-0 z-10"
                    >
                      <svg
                        class="w-4 h-4 text-[#315997]"
                        fill="none"
                        stroke="currentColor"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                  ...
                  <div class="flex-1 rounded-2xl bg-white border border-[#e3e2e0] p-3">
                    {#if entry.type === 'harvest'}
                      ...
                      <p class="text-xs text-[#737873] mt-0.5">
                        Récolte
                        {#if entry.recordedBy}
                          · par <span class="font-medium text-[#424844]">{entry.recordedBy}</span
                          >{/if}
                        {#if entry.terrainCondition}
                          · {entry.terrainCondition}{/if}
                      </p>
                    {:else if entry.type === 'transformation'}
                      <p class="text-sm font-semibold text-[#1a1c1a]">
                        {entry.inputProduct?.name ?? 'Brut'} -> {entry.outputProduct?.name ??
                          'Transformé'}
                      </p>
                      <p class="text-xs text-[#737873] mt-0.5">
                        -{entry.inputQuantity}
                        {entry.inputUnit} / +{entry.outputQuantity}
                        {entry.outputUnit}
                        {#if entry.recordedBy}
                          · par <span class="font-medium text-[#424844]">{entry.recordedBy}</span
                          >{/if}
                      ...
                      {#if entry.estimatedWaste > 0}
                        <p class="text-xs text-[#964824] mt-1">
                          Pertes estimées: {entry.estimatedWaste}
                        </p>
                      {/if}
                    {:else}
                      <p class="text-sm font-semibold text-[#1a1c1a]">
                      ...
                      <p class="text-xs text-[#737873] mt-0.5">
                        {orderStatusLabel(entry.status)} · {entry.items?.length ?? 0} ligne{(entry
                          .items?.length ?? 0) > 1
                          ? 's'
                      ...
                      <div class="mt-2 space-y-1">
                        {#each (entry.items ?? []).slice(0, 3) as item}
                          <p class="text-xs text-[#1a1c1a]">
                            • {item.productName} — {item.quantity} × {formatMoney(item.unitPrice)} = {formatMoney(
                              item.subtotal
                            )}
                          </p>
                        {/each}
                        {#if (entry.items?.length ?? 0) > 3}
                          <p class="text-xs text-[#737873]">
                            + {(entry.items?.length ?? 0) - 3} autre(s) ligne(s)
                          </p>
                        {/if}
                      </div>
                      <p class="text-xs text-[#424844] mt-1">
                        Contact: {entry.customerEmail} · {entry.customerPhone}
                      </p>
                    {/if}
                    {#if entry.notes}

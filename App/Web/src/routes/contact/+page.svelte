<script>
  ...
  const defaultSubject = $derived(
    $page.url.searchParams.get('subject') ?? 'Message depuis le site'
  );
</script>
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Contactez la Ferme de l'Humus pour prendre rendez-vous au labo ou pour toute question."
  />
</svelte:head>
  ...
  <div class="max-w-[960px] mx-auto">
    <section class="mb-lg text-center">
      ...
      <p class="font-body-lg text-body-lg text-on-surface-variant">
        Une question, une commande, ou vous souhaitez visiter notre labo à Ailly-sur-Somme ? Contact
        direct par téléphone, e-mail ou formulaire.
      </p>
      <div class="mt-md">
        <a
          href={quoteMailto}
    ...
    <section class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-lg">
      <a
        ...
        class="bg-white rounded-xl border border-outline-variant/10 shadow-ambient p-md hover:bg-surface-container-low transition-colors"
      >
      ...
      <a
        href="https://maps.google.com/?q=15+bis+rue+du+Pont+80470+Ailly-sur-Somme"
        target="_blank"
        rel="noreferrer"
        class="bg-white rounded-xl border border-outline-variant/10 shadow-ambient p-md hover:bg-surface-container-low transition-colors"
      >
    ...
    {#if form?.success}
      <div class="bg-[#d0e8d7] border border-[#172c21]/20 rounded-xl p-6 text-center mb-lg">
        <span class="material-symbols-outlined text-4xl text-[#172c21] mb-2 block"
          >check_circle</span
        >
        <p class="font-bold text-[#172c21]">Message envoyé !</p>
        <p class="text-sm text-[#172c21]/80 mt-1">
    ...
    {:else}
      ...
      >
        ...
        <div>
          <label for="message" class="block text-sm font-medium text-[#1a1c1a] mb-1"
            >Message *</label
          >
          <textarea
            id="message"
        ...
        <p class="text-xs text-center text-on-surface-variant">
          ...
          <a href={`mailto:${CONTACT_EMAIL}`} class="text-secondary font-medium hover:underline"
            >{CONTACT_EMAIL}</a
          >
        </p>
      </form>

...
import {
  O as ORDER_PICKUP_LOCATIONS,
  e as ORDER_REFERRAL_SOURCES,
} from '../../../chunks/constants.js';
function ProductCard($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let { product } = $$props;
    const stockLabel = derived(() =>
      product.stock ? `${product.stock.quantity ?? 0} ${product.stock.unit ?? ''}`.trim() : null
    );
    $$renderer2.push(
      `<div class="bg-white rounded-xl shadow-ambient overflow-hidden group hover:-translate-y-1 transition-transform duration-300 flex flex-col"><div class="aspect-[4/3] overflow-hidden"><img${attr('src', product.image || 'https://placehold.co/600x450/efeeeb/172c21?text=Photo+bientôt')}${attr('alt', product.name)} class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy"/></div> <div class="p-md flex flex-col gap-sm flex-1"><div class="flex justify-between items-start"><div class="flex-1 min-w-0 pr-2">`
    );
    if (product.badge) {
      $$renderer2.push('<!--[0-->');
      $$renderer2.push(
        `<span class="font-label-caps text-label-caps text-secondary uppercase tracking-widest mb-xs block">${escape_html(product.badge)}</span>`
      );
    } else {
      $$renderer2.push('<!--[-1-->');
      $$renderer2.push(
        `<span class="font-label-caps text-label-caps text-on-surface-variant uppercase tracking-widest mb-xs block">${escape_html(product.category)}</span>`
      );
    }
    $$renderer2.push(
      `<!--]--> <h3 class="font-h3 text-h3 text-primary">${escape_html(product.name)}</h3></div> <span class="font-h3 text-h3 text-primary whitespace-nowrap">${escape_html(product.price.toFixed(2))}€</span></div> <p class="font-body-md text-on-surface-variant line-clamp-2 flex-1">${escape_html(product.description)}</p> `
    );
    if (stockLabel()) {
      $$renderer2.push('<!--[0-->');
      $$renderer2.push(
        `<p class="text-xs text-on-surface-variant">En stock: <span class="font-semibold text-primary">${escape_html(stockLabel())}</span></p>`
      );
    } else {
      $$renderer2.push('<!--[-1-->');
    }
    $$renderer2.push(
      `<!--]--> <button class="mt-base w-full bg-secondary text-white py-3 rounded-lg flex items-center justify-center gap-2 hover:opacity-90 active:scale-95 transition-all"><span class="material-symbols-outlined text-[20px]">add</span> <span class="font-label-caps uppercase tracking-widest text-[11px]">Ajouter au panier</span></button></div></div>`
    );
  });
}
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    function resolveProductImage(product) {
      ...
      const fallback = visualFallbacks.find(({ match }) =>
        match.every((token) => source.includes(token))
      );
        ...
        : [
            {
              ...
              price: 8.5,
              description:
                'Récolté à froid, ce miel exprime toute la diversité florale de nos lisières boisées.',
              image: '/images/ferme/produit-jus-pomme-1l.jpg',
            },
            {
              _id: '2',
              ...
              price: 3.2,
              description:
                'Variété ancienne vigoureuse, cultivée sans intrants chimiques. Idéal pour votre potager.',
              image: '/images/ferme/serre-caisses.jpg',
            },
            {
              _id: '3',
              ...
              price: 5,
              description:
                'Petites, sucrées et intensément parfumées. Cueillies à maturité chaque matin.',
              image: '/images/ferme/produit-confiture-prunes.jpg',
            },
            {
              _id: '4',
              ...
              price: 14,
              description:
                'Un assortiment de 4 à 5 variétés de légumes fraîchement récoltés cette semaine.',
    ...
    const filtered = derived(() => allProducts());
    let cart = [];
    ...
    const cartPayload = derived(() =>
      cart.map(({ product, quantity }) => ({ productId: product._id, quantity }))
    );
...
puis récupérez votre sélection sur marché ou sur rendez-vous.</p></section> <section class="flex flex-wrap gap-sm mb-lg" role="group" aria-label="Filtrer les produits"><!--[-->`);
    const each_array = ensure_array_like(categories());
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      let cat = each_array[$$index];
      $$renderer2.push(
        `<button${attr_class(`px-6 py-2 rounded-full font-medium transition-all ${stringify(activeCategory === cat ? 'bg-primary-container text-on-primary' : 'bg-surface-container text-on-surface hover:bg-surface-container-high')}`)}>${escape_html(cat)}</button>`
      );
    }
    $$renderer2.push(
      `<!--]--></section> <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-md"><!--[-->`
    );
    ...
    if (cartCount() > 0) {
      $$renderer2.push('<!--[0-->');
      $$renderer2.push(
        `<section class="px-margin-mobile md:px-lg mb-36" id="validation-panier"><div class="max-w-[1140px] mx-auto rounded-2xl border border-[#2D4236]/10 bg-white shadow-ambient overflow-hidden"><div class="grid lg:grid-cols-[1.1fr_0.9fr]"><div class="p-lg md:p-xl bg-[#f5f5dc]"><p class="font-label-caps text-secondary uppercase tracking-widest mb-sm">Votre sélection</p> <h2 class="font-h2 text-h2 text-primary mb-md">Confirmer la commande</h2> <div class="space-y-3"><!--[-->`
      );
      const each_array_2 = ensure_array_like(cart);
      for (let $$index_2 = 0, $$length = each_array_2.length; $$index_2 < $$length; $$index_2++) {
        let item = each_array_2[$$index_2];
        $$renderer2.push(
          `<div class="flex items-center justify-between gap-4 rounded-xl bg-white px-4 py-3 border border-[#2D4236]/10"><div class="min-w-0"><p class="font-medium text-primary truncate">${escape_html(item.product.name)}</p> <p class="text-sm text-on-surface-variant">${escape_html(item.quantity)} × ${escape_html(formatMoney(item.product.price))} = ${escape_html(formatMoney(item.quantity * item.product.price))}</p></div> <div class="flex items-center gap-2 shrink-0"><button type="button" class="w-8 h-8 rounded-full border border-[#2D4236]/15 text-[#2D4236] hover:bg-white transition-colors"${attr('aria-label', `Retirer un ${item.product.name}`)}>−</button> <button type="button" class="w-8 h-8 rounded-full border border-[#2D4236]/15 text-[#2D4236] hover:bg-white transition-colors"${attr('aria-label', `Ajouter un ${item.product.name}`)}>+</button></div></div>`
        );
      }
      $$renderer2.push(
        `<!--]--></div> <div class="mt-6 flex items-center justify-between rounded-xl bg-white px-4 py-3 border border-[#2D4236]/10"><span class="font-medium text-primary">Total estimé</span> <span class="font-bold text-secondary">${escape_html(formatMoney(cartTotal()))}</span></div> <p class="mt-4 text-sm text-on-surface-variant">La validation réserve les quantités en stock et transmet la commande à l'atelier.</p></div> <div class="p-lg md:p-xl"><form id="product-order-form" method="POST" class="space-y-4">`
      );
      if (form?.error) {
        $$renderer2.push('<!--[0-->');
        $$renderer2.push(
          `<p class="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">${escape_html(form.error)}</p>`
        );
      } else {
        $$renderer2.push('<!--[-1-->');
      }
      ...
      $$renderer2.push(
        `<!--]--></select></div> <div><label for="referralSource" class="block text-sm font-medium text-[#1a1c1a] mb-1">Par curiosité, comment avez-vous entendu parler de nous ?</label> <select id="referralSource" name="referralSource" class="w-full rounded-xl border border-[#e3e2e0] bg-[#f4f3f1] px-4 py-3 text-sm focus:border-[#172c21] focus:outline-none">`
      );

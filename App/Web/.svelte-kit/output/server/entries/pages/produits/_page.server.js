...
import { O as ORDER_PICKUP_LOCATIONS, e as ORDER_REFERRAL_SOURCES, a as CONTACT_EMAIL } from "../../../chunks/constants.js";
import { g as getWriteClient } from "../../../chunks/sanityWrite.js";
import { f as fetchStockEntriesByProduct, a as aggregateStockEntries } from "../../../chunks/stock.js";
...
function buildEmailText(order) {
  const lines = [
    ...
    "",
    "Produits:",
    ...order.items.map((item) => `- ${item.quantity} x ${item.productName} (${formatMoney(item.subtotal)})`),
    "",
...
const actions = {
  default: async ({ request }) => {
    if (!private_env.SANITY_API_WRITE_TOKEN) {
      return fail(500, {
    ...
    const [products, stockEntryLists] = await Promise.all([
      ...
      Promise.all(productIds.map(async (productId) => [productId, await fetchStockEntriesByProduct(client, productId)]))
    ]);
    ...
    for (const { productId, quantity } of cart) {
      ...
      if (aggregated.totalQuantity < quantity) {
        return fail(400, {
          error: `Stock insuffisant pour ${product.name}. Disponible: ${aggregated.totalQuantity} ${aggregated.unit ?? ""}`.trim()
        });
      }
      orderItems.push({
        productId,
    ...
    const orderNumber = `CMD-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10).replaceAll("-", "")}-${Math.random().toString(36).slice(2, 7).toUpperCase()}`;
    ...
    const smtpUser = private_env.SMTP_USER;
    const smtpPass = private_env.SMTP_PASS;

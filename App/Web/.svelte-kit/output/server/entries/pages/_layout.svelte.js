...
function Header($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<header${attr_class(`fixed top-0 left-0 w-full z-50 flex justify-between items-center px-4 md:px-5 py-3 md:py-4 transition-all duration-300 ${stringify(transparent() ? "bg-transparent border-b border-transparent" : "bg-[#FAF9F6]/95 backdrop-blur-md border-b border-[#2D4236]/10 shadow-[0px_4px_20px_rgba(45,66,54,0.05)]")}`)}><div class="flex items-center gap-4"><button${attr_class(`material-symbols-outlined cursor-pointer hover:opacity-80 transition-colors duration-300 md:hidden ${stringify(transparent() ? "text-white [filter:drop-shadow(0_1px_3px_rgba(0,0,0,0.4))]" : "text-[#2D4236]")}`)} aria-label="Menu">menu</button> <a href="/"${attr_class(`flex items-center gap-2 sm:gap-3 text-base sm:text-xl font-semibold italic font-newsreader tracking-tight transition-colors duration-300 ${stringify(transparent() ? "text-white [text-shadow:0_1px_6px_rgba(0,0,0,0.4)]" : "text-[#2D4236]")}`)}><img src="/images/ferme/logo-fh.png" alt="Logo de la Ferme de l'Humus" class="h-8 w-8 sm:h-10 sm:w-10 object-contain rounded-full bg-white/80 p-1 shadow-sm" loading="eager"/> <span class="hidden sm:inline">Ferme de l'Humus</span></a></div> <nav class="hidden md:flex gap-8 items-center"><!--[-->`);
    ...
    $$renderer2.push(`<!--]--></nav> <a href="/reserver" class="hidden sm:inline-flex bg-primary text-on-primary px-5 py-2 rounded-lg font-label-caps uppercase tracking-widest hover:opacity-90 active:scale-95 transition-all">Réserver</a></header> `);
    {
      $$renderer2.push("<!--[-1-->");
    }
    $$renderer2.push(`<!--]-->`);
...
function Footer($$renderer) {
  $$renderer.push(`<footer class="relative w-full py-12 px-5 flex flex-col items-center gap-6 text-center bg-[#F5F5DC] border-t border-[#2D4236]/10 font-newsreader"><a href="/" class="flex items-center gap-3 text-lg font-bold text-[#2D4236]"><img src="/images/ferme/logo-fh.png" alt="Logo de la Ferme de l'Humus" class="h-10 w-10 object-contain rounded-full bg-white p-1 shadow-sm" loading="lazy"/> <span>Ferme de l'Humus</span></a> <p class="max-w-md text-[#2D4236]/70 text-sm leading-relaxed">Microferme fruitière et agroécologique à Ailly-sur-Somme, en vente directe et sur les marchés.</p> <div class="flex flex-col sm:flex-row gap-2 sm:gap-5 items-center text-sm text-[#2D4236]/80"><a href="tel:+33789649894" class="hover:text-[#A0522D] transition-colors">07 89 64 98 94</a> <span class="hidden sm:inline text-[#2D4236]/30">•</span> <a href="mailto:fermehumus@gmail.com" class="hover:text-[#A0522D] transition-colors">fermehumus@gmail.com</a></div> <div class="flex flex-wrap justify-center gap-8"><a href="/mentions-legales" class="text-[#2D4236]/60 text-xs tracking-wide hover:text-[#A0522D] transition-colors">Mentions Légales</a> <a href="/politique-de-confidentialite" class="text-[#2D4236]/60 text-xs tracking-wide hover:text-[#A0522D] transition-colors">Politique de Confidentialité</a> <a href="/cgv" class="text-[#2D4236]/60 text-xs tracking-wide hover:text-[#A0522D] transition-colors">CGV</a></div> <p class="text-[#2D4236]/60 text-xs">© 2024 Ferme de l'Humus. Régénération &amp; Transparence.</p> <a href="/admin/login" aria-label="Acces equipe" title="Acces equipe" class="absolute left-3 bottom-3 inline-block h-4 w-4 rounded-full border border-[#2D4236]/20 opacity-15 transition-opacity hover:opacity-50 focus:opacity-70 focus:outline-none focus:ring-1 focus:ring-[#2D4236]/40"><span class="sr-only">Acces equipe</span></a></footer>`);
}
function _layout($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    head("12qhfyh", $$renderer2, ($$renderer3) => {
      ...
      $$renderer3.push(`<meta name="description" content="Une agriculture régénératrice au service de la biodiversité et de votre santé. Produits biologiques, séjours à la ferme et marchés locaux."/> <link rel="icon" href="/favicon.png"/> <link rel="apple-touch-icon" href="/favicon.png"/>`);
    });
    if (!isAdminRoute()) {
      $$renderer2.push("<!--[0-->");
      Header($$renderer2);

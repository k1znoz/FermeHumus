...
function FAB($$renderer, $$props) {
  ...
  if (count > 0) {
    $$renderer.push('<!--[0-->');
    $$renderer.push(
      `<span class="absolute -top-1 -right-1 bg-primary text-on-primary text-[10px] w-5 h-5 rounded-full flex items-center justify-center font-bold">${escape_html(count)}</span>`
    );
  } else {
    $$renderer.push('<!--[-1-->');
  }
